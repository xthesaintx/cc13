...
async getData() {
  ...
  activateListeners(html) {
    ...
    nativeHtml.querySelector('.btn-open-scene')?.addEventListener('click', this._onOpenScene.bind(this));
...
// ADD to v13
let sceneButtonHtml = '';
  if ((this._selectedSheet.type === 'location' || this._selectedSheet.type === 'region' || this._selectedSheet.type === 'shop') && selectedData.linkedScene) {
    sceneButtonHtml = `
...
// 
  ...
  async _addMemberToGroup(newMemberUuid) {
    ...
    if (!newMemberDoc) {
        ui.notifications.error("The item you are trying to add could not be found.");
        return;
    }
    ...
    const newMemberAsGroupMember = [{ uuid: newMemberUuid, type: newMemberDoc.getFlag("campaign-codex", "type") }];
